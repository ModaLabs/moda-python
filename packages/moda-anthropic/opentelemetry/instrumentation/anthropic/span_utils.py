...
async def aset_input_attributes(span, kwargs):
    ...
    # Moda conversation tracking
    try:
        from traceloop.sdk.conversation import compute_conversation_id, get_user_id

        # Normalize Anthropic format (system is separate param)
        normalized = []
        if kwargs.get("system"):
        ...
        if normalized:
            ...
            set_span_attribute(span, "moda.conversation_id", conversation_id)

        user_id = get_user_id()
        if user_id:
            set_span_attribute(span, "moda.user_id", user_id)
    except ImportError:
        # Moda SDK not installed, skip conversation tracking
        pass
